package nl.schereper.andrei.data

import android.util.Log
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import nl.schereper.andrei.interfaces.PokemonAPIService
import nl.schereper.andrei.mappers.PokemonDetailsMapper
import nl.schereper.andrei.mappers.PokemonListMapper
import nl.schereper.andrei.model.EvolutionNode
...
class DataStore @Inject constructor(
    ...
    private val _pokemonList = MutableStateFlow<List<nl.schereper.andrei.model.Pokemon>>(emptyList())
    val pokemonList: StateFlow<List<nl.schereper.andrei.model.Pokemon>> = _pokemonList

    private val _favoritePokemonList = MutableStateFlow<Set<nl.schereper.andrei.model.Pokemon>>(emptySet())
    val favoritePokemonList: StateFlow<Set<nl.schereper.andrei.model.Pokemon>> = _favoritePokemonList
    ...
    fun toggleFavorite(selectedPokemon: nl.schereper.andrei.model.Pokemon) {
    ...
    fun getPokemonDetailsById(id: Int, onResult: (nl.schereper.andrei.model.Pokemon?) -> Unit) {
        viewModelScope.launch {
            try {
    ...
    fun getEvolutionChainForPokemon(id: Int, onResult: (List<nl.schereper.andrei.model.Pokemon>?) -> Unit) {
        viewModelScope.launch {
            try {
    ...
    private fun parseEvolutionChain(node: EvolutionNode?): List<nl.schereper.andrei.model.Pokemon> {
        if (node == null) return emptyList()

        val result = mutableListOf<nl.schereper.andrei.model.Pokemon>()
